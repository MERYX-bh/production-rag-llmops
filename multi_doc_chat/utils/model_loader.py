...
class ApiKeyManager:
    ...
    def __init__(self):
        ...
        # Final check — OpenAI key is optional (only needed if provider=openai)
...
class ModelLoader:
    ...
    def load_embeddings(self):
        """
        Load and return embedding model based on provider in config.yaml.
        Supports: openai, google
        """
        try:
            embedding_cfg = self.config["embedding_model"]
            provider = embedding_cfg.get("provider", "google")
            ...
            if provider == "openai":
                return OpenAIEmbeddings(
                    model=model_name,
                    api_key=self.api_key_mgr.get("OPENAI_API_KEY", required=False) or os.getenv("OPENAI_API_KEY"),  # type: ignore
                )
